    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="attempts" className="space-y-4">
        <TabsList className="h-auto flex-wrap justify-start">
          <TabsTrigger value="attempts" data-testid="tab-verification-attempts"><FileClock className="mr-2 h-4 w-4" />{t('محاولات التحقق', 'Attempts')}</TabsTrigger>
          <TabsTrigger value="profiles" data-testid="tab-verification-profiles"><UserRoundCheck className="mr-2 h-4 w-4" />{t('الملفات والمراجعة', 'Profiles & review')}</TabsTrigger>
          <TabsTrigger value="policy" data-testid="tab-verification-policy"><ClipboardList className="mr-2 h-4 w-4" />{t('الضوابط', 'Controls')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="profiles" className="space-y-4">
          <Card><CardContent className="flex flex-col gap-3 pt-6 sm:flex-row sm:items-center"><div className="flex items-center gap-2"><label className="text-sm font-medium" htmlFor="profile-status">{t('الحالة', 'Status')}</label><select id="profile-status" value={profileStatus} onChange={event => setProfileStatus(event.target.value as typeof profileStatus)} className="h-9 rounded-md border bg-background px-3 text-sm" data-testid="select-profile-status">{statuses.map(status => <option key={status} value={status}>{status === 'all' ? t('كل الحالات', 'All statuses') : statusLabel(status, language)}</option>)}</select></div><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={profileUid} onChange={event => setProfileUid(event.target.value)} placeholder={t('معرف المستخدم الدقيق', 'Exact user ID')} className="pl-9" data-testid="input-profile-uid" /></div></CardContent></Card>
