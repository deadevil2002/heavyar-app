...
import { useVerification, useActionMutation } from '@/lib/api';
...
export default function Verification() {
  ...
  const { data, isLoading } = useVerification({ status: debouncedSearch || undefined, limit: 20, ...(cursor ? { cursor } : {}) });
  ...
  const handleAction = (item: any, status: string) => {
    triggerAction({
      ...
      title: t('حالة التحقق', 'Verification Status'),
      description: t('تأكيد قرار التحقق.', 'Confirm verification decision.'),
      payload: { status }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('التحقق', 'Verification')}</h1>
          <p className="text-muted-foreground mt-1">{t('مراجعة هويات المستخدمين والمزودين', 'Review user and provider identities')}</p>
        </div>
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="border-border hover:bg-transparent">
              ...
              <TableHead className="font-semibold text-foreground">{t('النوع', 'Type')}</TableHead>
              <TableHead className="font-semibold text-foreground">{t('المعرف الهدف', 'Target ID')}</TableHead>
            ...
            {isLoading ? (
              ...
              data?.items?.map((item: any) => (
                <TableRow key={item.id} className="border-border border-b last:border-0 hover:bg-muted/20">
                  ...
                  <TableCell className="font-mono text-xs text-muted-foreground">{item.targetId.substring(0, 8)}...</TableCell>
                  <TableCell>
                    ...
                    {item.status === 'pending' || item.status === 'needs_review' ? (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handleAction(item, 'approved')} className="text-emerald-500">
                            <CheckCircle className="mr-2 h-4 w-4" />
                            {t('اعتماد', 'Approve')}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleAction(item, 'rejected')} className="text-destructive">
