...
type Props = { item?: Record<string, unknown> | null; open: boolean; onOpenChange: (open: boolean) => void; title: string; description?: string; fields?: { label: string; value?: unknown; technical?: boolean }[] };

export function OperationDetails({ item, open, onOpenChange, title, description, fields }: Props) {
  const { language } = useAppState();
  const values = fields || Object.entries(item || {}).filter(([key]) => !['before', 'after', 'events', 'providerPayload'].includes(key)).map(([label, value]) => ({ label, value, technical: /uid|id/i.test(label) }));
  return (
