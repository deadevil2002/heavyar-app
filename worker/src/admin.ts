...
async function action(req: Request, env: Env, u: AdminUser, suppliedBody?: any) {
  ...
  if ((!raw?.data || !raw.updateTime) && !isDefaultVerificationPolicy && !(normalizedType === 'paymentGateway' && targetId === String(targetId)) && !(normalizedType === 'identityIntegrations' && targetId === 'nafath_rabet') && !(normalizedType === 'config' && can(u, 'staff.manage'))) return { error: 'Target not found', status: 404 };
...
export async function handleAdmin(req: Request, env: Env, user: AdminUser) {
  ...
  if (detailMatch) {
    const aliases: Record<string, string> = { request: 'equipmentRequests', provider: 'users', driver: 'driverProfiles', listing: 'equipment' };
    ...
    const [item] = await enrichAdminItems(env, collection, [{ id: decodeURIComponent(detailMatch[2]), ...redact(raw.data) }]);
    return { success: true, item: { ...item, technicalIds: { firestoreDocumentId: decodeURIComponent(detailMatch[2]) } } };
  }
