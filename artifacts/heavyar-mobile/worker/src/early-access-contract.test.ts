...
import { campaignProgress } from './early-access-campaign-delivery';
...
describe('Early Access real adapter, signed shared webhook and REST preconditions', () => {
  ...
  test('shared signed webhook accepted/delivered/bounced/complained/failed projects then preview excludes failures; replay safe', async () => {
    ...
    const deliveredName = `${prefix}${EA.deliveries}/campaign-delivery`;
    expect(docs.get(deliveredName).fields.deliveryStatus.stringValue).toBe('delivered');
    const deliveredRecord = structuredClone(docs.get(deliveredName));
    expect(typeof deliveredRecord.fields.deliveryEventAt.timestampValue).toBe('string');
    expect(typeof deliveredRecord.fields.deliveryUpdatedAt.timestampValue).toBe('string');
    const deliveredProgress = await campaignProgress(earlyAccessStore(env, actor), 'campaign');
    const commitsAfterDelivered = commits.length;
    expect((await campaignEvent('delivered', 'campaign-delivered')).status).toBe(200);
    expect(commits.length).toBe(commitsAfterDelivered);
    expect(docs.get(deliveredName)).toEqual(deliveredRecord);
    expect(await campaignProgress(earlyAccessStore(env, actor), 'campaign')).toEqual(deliveredProgress);
