...
export async function handleSeoPublic(req: Request, store: Pick<SeoStore, 'read'>): Promise<Response> {
  ...
  try {
    ...
    if (!state.currentPublishedId) return json({ success: false, errorCode: 'SEO_NOT_PUBLISHED', error: 'No SEO configuration has been published.' }, 404);
