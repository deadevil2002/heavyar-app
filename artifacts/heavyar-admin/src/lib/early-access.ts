...
export function buildPreviewPayload(id: string, subscriberIds: Set<string>, language?: string, country?: string) {
  return { id, subscriberIds: Array.from(subscriberIds), language, country };
}
...
export function usePreviewCampaign() {
  return useMutation({
    mutationFn: ({ id, subscriberIds, language, country }: { id: string; subscriberIds: string[]; language?: string; country?: string }) =>
      fetchApi<PreviewResponse>(`/early-access/campaigns/${id}/preview`, {
        method: 'POST',
        body: JSON.stringify({ subscriberIds, language, country }),
      }),
  });
}
