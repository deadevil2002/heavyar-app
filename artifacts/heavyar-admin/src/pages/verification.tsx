  ...
  const submit = (values: PolicyValues) => {
    ...
    if (!parsed.success) {
      parsed.error.issues.forEach(issue => {
        ...
        if (typeof field === 'string') form.setError(field as keyof PolicyValues, { message: userErrorMessage('INVALID_INPUT', language) });
      });
      return;
    }
    action.mutate({
      ...
      <CardContent>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(submit)} className="space-y-5">
            <FormField control={form.control} name="enabled" render={({ field }) => <FormItem className="flex min-w-0 items-center justify-between gap-4 rounded-md border p-4"><div className="min-w-0 flex-1 break-words"><FormLabel>{t('تفعيل سياسة التحقق', 'Enable verification policy')}</FormLabel><FormDescription>{t('عند الإيقاف، لا يتم فرض قواعد التحقق التلقائية.', 'When disabled, automatic verification rules are not enforced.')}</FormDescription></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} data-testid="switch-policy-enabled" /></FormControl></FormItem>} />
            <FormField control={form.control} name="requireCustomerIdentityVerification" render={({ field }) => <FormItem className="flex min-w-0 items-center justify-between gap-4 rounded-md border p-4"><div className="min-w-0 flex-1 break-words"><FormLabel>{t('طلب هوية العميل', 'Require customer identity')}</FormLabel><FormDescription>{t('يلزم التحقق قبل إجراءات العميل المحمية.', 'Requires verification before protected customer actions.')}</FormDescription></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} data-testid="switch-customer-identity-required" /></FormControl></FormItem>} />
            ...
            {action.isError && <p className="text-sm text-destructive" data-testid="status-policy-error">{userErrorMessage(action.error, language)}</p>}
