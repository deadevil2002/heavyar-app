...
import { formatMinor } from '@/components/rental-v2-details';
...
export default function Equipment() {
  ...
  const pricingText = (equipment: Equipment) => {
    if (equipment.pricingModelVersion === 2 && equipment.pricing) {
      const { pricing } = equipment;
      const rates = [
        pricing.hourly.enabled && pricing.hourly.amountMinor != null
          ? `${formatMinor(pricing.hourly.amountMinor, pricing.currency, undefined, language)} / ${t('ساعة', 'hour')}`
          : null,
        pricing.daily.enabled && pricing.daily.amountMinor != null
          ? `${formatMinor(pricing.daily.amountMinor, pricing.currency, undefined, language)} / ${t('يوم', 'day')}`
          : null,
      ].filter(Boolean);
      return rates.length ? rates.join(' · ') : '—';
    }
    const legacy = equipment.pricePerDay ?? equipment.dailyRate ?? equipment.rate;
    return legacy != null ? `${legacy} SAR / ${t('يوم', 'day')} (${t('قديم', 'legacy')})` : '—';
  };
  const columns: OperationsColumn<Equipment>[] = [
    ...
    { key: 'rate', label: t('التسعير', 'Pricing'), render: e => <span className="text-xs">{pricingText(e)}</span> },
